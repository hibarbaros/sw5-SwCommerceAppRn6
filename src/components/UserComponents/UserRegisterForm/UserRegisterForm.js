...
import {useListMutation} from '../../../utils/hooks/useCustomer';
...
export default function UserRegisterForm() {
	...
	const {addCustomerLoading, mutateAddCustomer} = useListMutation();
		...
		mutateAddCustomer(value, {
			onSuccess: (data, variables, context) => {
				if (data) {
					...
					navigation.dispatch(
						StackActions.replace(AppRoutes.PROFILE)
					);
				}
			},
		});
						...
						{({
							...
							<>
								...
								<View marginV-s2>
									...
									/>
									{touched.billingsalutation &&
										errors.billingsalutation && (
											<FormErrorLabel
												errorMessage={
													errors.billingsalutation
												}
